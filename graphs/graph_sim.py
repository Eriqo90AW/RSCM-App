...
class GraphSim:
    ...
    def getMaxValue(self):
        max_array = np.maximum(np.maximum(np.maximum(self.lines_1, self.lines_2), self.lines_3), self.lines_avg)
        result = np.max(max_array)
        self.main_window.ui.label_graph_max.setText(f'Max : {round(result, 2)} cm')
    
    # method to set a label to show the min value
    def getMinValue(self):
        min_array = np.minimum(np.minimum(np.minimum(self.lines_1, self.lines_2), self.lines_3), self.lines_avg)
        result = np.min(min_array)
        self.main_window.ui.label_graph_min.setText(f'Min : {round(result, 2)} cm')
